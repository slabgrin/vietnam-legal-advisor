#!/usr/bin/env python3
"""
scraper.py — Fetches latest Vietnamese legal updates.
...
Return a JSON array of objects with EXACTLY these fields:
  - id          : unique slug, e.g. "decree-12-2026-nd-cp"
  - title       : English title
  - doc_number  : official number, e.g. "Decree 12/2026/NĐ-CP"
  ...
  - url         : PRIORITY — vbpl.vn URL if available
                  (e.g. https://vbpl.vn/TW/Pages/vbpqen-toanvan.aspx?ItemID=XXXXX),
                  then thuvienphapluat.vn, then any official source, else null
...
def fetch_via_sonar(client, model, lookback_days):
    response = client.chat.completions.create(
        model=model,
        messages=[{"role": "user", "content": SONAR_PROMPT.format(days=lookback_days)}],
        temperature=0.1,
    )
    ...
    for item in soup.select("div.vbTitle, li.vbTitle, .title-vb"):
        try:
            ...
            results.append({
                ...
                "url": url, "url_vn": url, "_source": "vbpl_scrape",
            })
        except Exception:
            continue
    return results
...
def main():
    ...
    laws = []
    if api_key:
        print(f"[scraper] Querying Perplexity ({config['llm']['models']['summarizer']}) for last {lookback} days...")
        client = OpenAI(api_key=api_key, base_url=config["llm"]["base_url"])
        try:
            laws = fetch_via_sonar(client, config["llm"]["models"]["summarizer"], lookback)
    ...
    print(f"[scraper] {len(new_laws)} new laws after dedup.")
